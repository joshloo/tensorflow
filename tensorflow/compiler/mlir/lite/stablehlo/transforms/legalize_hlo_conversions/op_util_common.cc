...
bool IsSamePaddingOnDim(const DimPadding& pad, int64_t out_dim, int64_t in_dim,
                        int64_t stride_dim) {
  const int64_t pad_diff = pad.Hi() - pad.Lo();
  if (pad_diff > 1 || pad_diff < 0) {
    return false;
  }
  const int64_t expected_out = std::ceil(in_dim / (float)stride_dim);
  return expected_out == out_dim;
}
