...
absl::StatusOr<std::vector<xla::ifrt::Device*>> GetAssignedIfrtDevices(
    ...
    std::optional<std::vector<int>> device_assignment_attr) {
  ...
  for (auto* device : ifrt_client.addressable_devices()) {
  ...
  if (!device_assignment_attr || device_assignment_attr->empty() ||
  ...
  for (auto coord_attr : *device_assignment_attr) {
  ...
  for (auto* device : ifrt_client.addressable_devices()) {
    ...
    if (device->Attributes().map().contains("coords") &&
      ...
      VLOG(3) << "Adding coords and core_on_chip attributes:"
              << device->DebugString();
      auto coords_list = std::get<xla::ifrt::AttributeMap::Int64ListValue>(
          device->Attributes().map().at("coords"));
      auto core_on_chip = std::get<xla::ifrt::AttributeMap::Int64Value>(
          device->Attributes().map().at("core_on_chip"));
      grid = GridCoords(coords_list.value.at(0), coords_list.value.at(1),
                        coords_list.value.at(2), core_on_chip.value);
    } else {
    ...
    if (!devices_from_attribute.contains(grid)) {
      VLOG(1) << "Device coordinates " << grid.ToString()
              << " does not match any TPU device assigned "
              << absl::StrJoin(*device_assignment_attr, " ");
      continue;
    }
    ifrt_devices.push_back({.device = device, .grid = grid});
  }
  ...
  absl::c_sort(ifrt_devices, [&](const auto& lhs, const auto& rhs) {
    return devices_from_attribute.at(lhs.grid) <
           devices_from_attribute.at(rhs.grid);
  });

  std::vector<xla::ifrt::Device*> result;
  result.reserve(ifrt_devices.size());
  for (auto& [device, _] : ifrt_devices) {
    result.push_back(device);
    VLOG(3) << "Device: " << device->DebugString() << " is assigned";
  }
  return result;
}
