      ...
      if (ctx->index_mapper()) {
        if (input_contexts_[0].index_mapper() == nullptr) {
          IndexMapperFn new_index_mapper1 =
            ...
            if (pidx >= left_cardinality + right_cardinality) {
              return absl::OutOfRangeError("Running out of elements.");
            }
            if (pidx >= left_cardinality) {
        ...
        absl::StatusOr<size_t> shuffled_index_or =
            absl::NotFoundError("default");

        while (absl::IsNotFound(shuffled_index_or.status())) {
          shuffled_index_or = ctx->index_mapper()(element_count_++);
          if (absl::IsOutOfRange(shuffled_index_or.status())) {
            *end_of_sequence = true;
            return absl::OkStatus();
          }
          if (!absl::IsNotFound(shuffled_index_or.status()) &&
              !shuffled_index_or.ok()) {
            return shuffled_index_or.status();
          }
        }

        // Routes the shuffle index to the correct input iterator.
        const size_t& shuffled_index = shuffled_index_or.value();
        bool temp_end_of_sequence = false;
        absl::Status status = absl::OkStatus();
        if (shuffled_index < dataset()->input_cardinality_) {
