...
void ApplyIndexingOp::build(OpBuilder& builder, OperationState& result,
  ...
  IndexingMap indexing_map_copy = indexing_map;
  indexing_map_copy.ClearConstraints();
  IndexingMapAttr indexing_map_attr =
      IndexingMapAttr::get(builder.getContext(), indexing_map_copy);
...
// Parser a comma-separated list of operands, ex: %d1, %d2
...
llvm::SmallVector<int64_t> ApplyIndexingOp::getLowerBounds() {
  auto lower_bounds = SmallVector<int64_t>();
...
llvm::SmallVector<int64_t> ApplyIndexingOp::getUpperBounds() {
  auto upper_bounds = SmallVector<int64_t>();
